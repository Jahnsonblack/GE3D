...
from scene.dataset_readers import sceneLoadTypeCallbacks,GenerateRandomCameras,GeneratePurnCameras,GenerateCircleCameras, GenerateRecoCameras
...
class Scene:
    ...
    def refine_save(self, iteration):
        point_cloud_path = os.path.join(self.model_path, "point_cloud/refine_iteration_{}".format(iteration))
        self.gaussians.save_ply(os.path.join(point_cloud_path, "point_cloud.ply"))
    ...
    def getReconTrainCameras(self, circle_size: list = [4], thetas: list = [90], scale: float = 1.0,):
        SSAA = False
        cam_infos = GenerateRecoCameras(self.pose_args, circle_size, thetas, scale)
        train_cameras = {}
        #print(self.resolution_scales)
        # for resolution_scale in self.resolution_scales:
        #     train_cameras[resolution_scale] = cameraList_from_RcamInfos(cam_infos, resolution_scale, self.pose_args, SSAA=True)      
        #return train_cameras[scale]
        return cameraList_from_RcamInfos(cam_infos, scale, self.pose_args, SSAA=True)   
