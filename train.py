...
def prepare_embeddings(guidance_opt, guidance):
    ...
    embeddings['neg'] = guidance.get_text_embeds([guidance_opt.negative])

    for d in ['front', 'side', 'back']:
        embeddings[d] = guidance.get_text_embeds([f"{guidance_opt.text}, {d} view"])
    embeddings['uncond'] = guidance.get_text_embeds(guidance_opt.inverse_text)
    return embeddings

def prepare_embeddings_head(guidance_opt, guidance):
    embeddings = {}
    ...
    for d in ['front', 'side', 'back']:
        embeddings[d] = guidance.get_text_embeds([f"The {d} view of {guidance_opt.text}"])
    embeddings['uncond'] = guidance.get_text_embeds(guidance_opt.inverse_text)
    return embeddings

def guidance_setup(guidance_opt):
    if guidance_opt.guidance=="SD":
        from guidance.GE3D import StableDiffusion
    ...
    if guidance_opt.use_back:
        embeddings = prepare_embeddings_head(guidance_opt, guidance)
    else:    
...
def training(dataset, opt, pipe, gcams, guidance_opt, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, save_video):
    guidance_opt.iterations = opt.iterations
    first_iter = 0
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        text_z_inverse =torch.cat([embeddings['neg'],embeddings['uncond']], dim=0)
        ...
        if guidance_opt.perpneg:
            loss = guidance.train_step_perpneg(torch.stack(text_z_, dim=1), images, 
                                                ...
                                                guidance_opt=guidance_opt,opt=opt,as_latent=_aslatent)
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_checkpoint_ratio", type=int, default=2)
    parser.add_argument("--save_video", type=bool, default=False)
    parser.add_argument("--quiet", action="store_true")
    parser.add_argument("--checkpoint_iterations", nargs="+", type=int, default=[2500])
    ...
    args.checkpoint_iterations = [k * op.iterations // args.save_checkpoint_ratio for k in range(1, args.save_checkpoint_ratio)] +[op.iterations]
